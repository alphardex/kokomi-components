...
var adaptive_luminance_default =
...
var BoxBlurMaterial = class extends ShaderMaterial3 {
  ...
  get worldDistanceThreshold() {
    return -orthographicDepthToViewZ(
      Number(this.defines.DISTANCE_THRESHOLD),
      this.near,
      this.far
    );
  }
...
var circle_of_confusion_default =
...
import {
  NoBlending as NoBlending6,
  ShaderMaterial as ShaderMaterial6,
  Uniform as Uniform6,
} from "three";
...
import {
  BasicDepthPacking as BasicDepthPacking3,
...
var depth_downsampling_default =
...
var depth_downsampling_default2 =
...
import {
  NoBlending as NoBlending11,
  ShaderMaterial as ShaderMaterial11,
  Uniform as Uniform11,
  Vector2 as Vector26,
} from "three";

// src/materials/glsl/convolution.downsampling.frag
var convolution_downsampling_default =
...
var convolution_downsampling_default2 =
...
import {
  ...
  ShaderMaterial as ShaderMaterial12,
  Uniform as Uniform12,
  Vector2 as Vector27,
} from "three";

// src/materials/glsl/edge-detection.frag
var edge_detection_default =
...
import {
  ...
  NoBlending as NoBlending13,
  PerspectiveCamera as PerspectiveCamera5,
  ...
  setShaderParts(shaderParts) {
    ...
    this.fragmentShader = effect_default
      .replace(
        ...
        (_a = shaderParts.get(EffectShaderSection.FRAGMENT_HEAD)) != null
          ? _a
          : ""
      )
      .replace(
        EffectShaderSection.FRAGMENT_MAIN_UV,
        (_b = shaderParts.get(EffectShaderSection.FRAGMENT_MAIN_UV)) != null
          ? _b
          : ""
      )
      .replace(
        EffectShaderSection.FRAGMENT_MAIN_IMAGE,
        (_c = shaderParts.get(EffectShaderSection.FRAGMENT_MAIN_IMAGE)) != null
          ? _c
          : ""
      );
    this.vertexShader = effect_default2
      .replace(
        ...
        (_d = shaderParts.get(EffectShaderSection.VERTEX_HEAD)) != null
          ? _d
          : ""
      )
      .replace(
        EffectShaderSection.VERTEX_MAIN_SUPPORT,
        (_e = shaderParts.get(EffectShaderSection.VERTEX_MAIN_SUPPORT)) != null
          ? _e
          : ""
      );
    this.needsUpdate = true;
...
import {
  NoBlending as NoBlending14,
  ShaderMaterial as ShaderMaterial14,
  Uniform as Uniform14,
  Vector2 as Vector29,
} from "three";

// src/materials/glsl/convolution.gaussian.frag
var convolution_gaussian_default =
...
var convolution_gaussian_default2 =
...
import {
  NoBlending as NoBlending15,
  ShaderMaterial as ShaderMaterial15,
  Uniform as Uniform15,
} from "three";

// src/materials/glsl/convolution.god-rays.frag
var convolution_god_rays_default =
...
import {
  NoBlending as NoBlending16,
  REVISION as REVISION5,
  ShaderMaterial as ShaderMaterial16,
...
import {
  NoBlending as NoBlending19,
  ShaderMaterial as ShaderMaterial19,
  Uniform as Uniform19,
  Vector2 as Vector211,
} from "three";

// src/materials/glsl/smaa-weights.frag
var smaa_weights_default =
...
import {
  ...
  ShaderMaterial as ShaderMaterial20,
...
var SSAOMaterial = class extends ShaderMaterial20 {
  ...
  set worldDistanceThreshold(value) {
    this.distanceThreshold = viewZToOrthographicDepth(
      -value,
      this.near,
      this.far
    );
  ...
  set distanceFalloff(value) {
    this.uniforms.distanceCutoff.value.y = Math.min(
      Math.max(this.distanceThreshold + value, 0),
      1
    );
  }
  ...
  set worldDistanceFalloff(value) {
    this.distanceFalloff = viewZToOrthographicDepth(
      -value,
      this.near,
      this.far
    );
  ...
  set worldProximityThreshold(value) {
    this.proximityThreshold = viewZToOrthographicDepth(
      -value,
      this.near,
      this.far
    );
  ...
  set proximityFalloff(value) {
    this.uniforms.proximityCutoff.value.y = Math.min(
      Math.max(this.proximityThreshold + value, 0),
      1
    );
  }
  ...
  set worldProximityFalloff(value) {
    this.proximityFalloff = viewZToOrthographicDepth(
      -value,
      this.near,
      this.far
    );
...
var convolution_upsampling_default =
...
var convolution_upsampling_default2 =
...
var Resolution = class extends EventDispatcher {
  ...
  copy(resolution) {
    ...
    this.preferredSize.set(
      resolution.preferredWidth,
      resolution.preferredHeight
    );
    this.dispatchEvent({ type: "change" });
    this.resizable.setSize(this.baseSize.width, this.baseSize.height);
  }
...
var DepthPickingPass = class extends DepthCopyPass {
  constructor({
    depthPacking = RGBADepthPacking4,
...
import {
  BasicDepthPacking as BasicDepthPacking12,
...
function integrateEffect(prefix, effect, data) {
  ...
  const mainImageExists =
    fragmentShader !== void 0 && /mainImage/.test(fragmentShader);
  const mainUvExists =
    fragmentShader !== void 0 && /mainUv/.test(fragmentShader);
  data.attributes |= effect.getAttributes();
  if (fragmentShader === void 0) {
    throw new Error(`Missing fragment shader (${effect.name})`);
  } else if (
    mainUvExists &&
    (data.attributes & EffectAttribute.CONVOLUTION) !== 0
  ) {
    throw new Error(
      `Effects that transform UVs are incompatible with convolution effects (${effect.name})`
    );
  } else if (!mainImageExists && !mainUvExists) {
    throw new Error(
      `Could not find mainImage or mainUv function (${effect.name})`
    );
  } else {
    ...
    let fragmentMainImage =
      (_c = shaderParts.get(EffectShaderSection.FRAGMENT_MAIN_IMAGE)) != null
        ...
        : "";
    let vertexHead =
      (_d = shaderParts.get(EffectShaderSection.VERTEX_HEAD)) != null ? _d : "";
...
`;
    ...
    if (vertexShader !== null && /mainSupport/.test(vertexShader)) {
      ...
      for (const m2 of vertexShader.matchAll(
        /(?:varying\s+\w+\s+([\S\s]*?);)/g
      )) {
    ...
    names.delete("for");
    names.delete("if");
    effect.uniforms.forEach((val, key) =>
      data.uniforms.set(
        prefix + key.charAt(0).toUpperCase() + key.slice(1),
        val
      )
    );
    effect.defines.forEach((val, key) =>
      data.defines.set(prefix + key.charAt(0).toUpperCase() + key.slice(1), val)
    );
    ...
    if (mainImageExists) {
      if (
        effect.inputColorSpace !== null &&
        effect.inputColorSpace !== data.colorSpace
      ) {
        fragmentMainImage +=
          effect.inputColorSpace === sRGBEncoding4
      ...
      if (
        (data.attributes & EffectAttribute.DEPTH) !== 0 &&
        depthParamRegExp.test(fragmentShader)
      ) {
  ...
  updateMaterial() {
    ...
    for (const effect of this.effects) {
      if (effect.blendMode.blendFunction === BlendFunction.DST) {
        ...
        throw new Error(
          `Convolution effects cannot be merged (${effect.name})`
        );
      } else {
    ...
    let fragmentMainImage = data.shaderParts.get(
      EffectShaderSection.FRAGMENT_MAIN_IMAGE
    );
    let fragmentMainUv = data.shaderParts.get(
      EffectShaderSection.FRAGMENT_MAIN_UV
    );
    ...
    for (const blendMode of data.blendModes.values()) {
      fragmentHead +=
        blendMode
          .getShaderCode()
          .replace(blendRegExp, `blend${blendMode.blendFunction}`) + "\n";
    }
    ...
    data.shaderParts.set(
      EffectShaderSection.FRAGMENT_MAIN_IMAGE,
      fragmentMainImage
    );
    data.shaderParts.set(EffectShaderSection.FRAGMENT_MAIN_UV, fragmentMainUv);
    data.shaderParts.forEach((value, key, map) =>
      map.set(key, value == null ? void 0 : value.trim().replace(/^#/, "\n#"))
    );
...
var EffectComposer = class {
  ...
  setRenderer(renderer) {
    ...
    if (renderer !== null) {
      ...
      if (
        frameBufferType === UnsignedByteType12 &&
        renderer.outputEncoding === sRGBEncoding8
      ) {
  ...
  createBuffer(depthBuffer, stencilBuffer, type, multisampling) {
    ...
    const size =
      renderer === null
        ? new Vector217()
        : renderer.getDrawingBufferSize(new Vector217());
    const options = {
    ...
    if (multisampling > 0) {
      renderTarget =
        Number(REVISION6.replace(/\D+/g, "")) < 138
          ? new WebGLMultisampleRenderTarget(size.width, size.height, options)
          : new WebGLRenderTarget13(size.width, size.height, options);
...
var GaussKernel = class {
  ...
  generate(kernelSize, edgeBias) {
    ...
    if (linearWeightSum !== 0) {
      for (
        let i = 0, l = linearWeights.length, s = 1 / linearWeightSum;
        i < l;
        ++i
      ) {
        linearWeights[i] *= s;
...
var Effect = class extends EventDispatcher3 {
  ...
  dispose() {
    for (const key of Object.keys(this)) {
      const property = this[key];
      const isDisposable =
        property instanceof WebGLRenderTarget14 ||
        property instanceof Material2 ||
        property instanceof Texture2 ||
        property instanceof Pass;
      if (isDisposable) {
        this[key].dispose();
      }
...
var brightness_contrast_default =
...
var ColorDepthEffect = class extends Effect {
  constructor({ blendFunction, bits = 16 } = {}) {
    super("ColorDepthEffect", color_depth_default, {
      blendFunction,
      uniforms: /* @__PURE__ */ new Map([["factor", new Uniform27(1)]]),
    });
    this.bits = 0;
    this.bitDepth = bits;
  }
...
var chromatic_aberration_default =
...
var chromatic_aberration_default2 =
...
var GammaCorrectionEffect = class extends Effect {
  constructor({ blendFunction = BlendFunction.SRC, gamma = 2 } = {}) {
    super("GammaCorrectionEffect", gamma_correction_default, {
      blendFunction,
      uniforms: /* @__PURE__ */ new Map([["gamma", new Uniform31(gamma)]]),
    });
  }
};
...
var GodRaysEffect = class extends Effect {
  constructor(
    ...
    super("GodRaysEffect", god_rays_default, {
      blendFunction,
      attributes: EffectAttribute.DEPTH,
      uniforms: /* @__PURE__ */ new Map([["map", new Uniform33(null)]]),
    });
    this.camera = camera;
...
var LUT1DEffect = class extends Effect {
  ...
  set lut(value) {
    ...
    if (
      value !== null &&
      (value.type === FloatType4 || value.type === HalfFloatType)
    ) {
...
// @todo Remove when postprocessing v7 lands.
export class DataTexture3D2 extends DataTexture3D {}
  ...
  set lut(value) {
    ...
    if (this.lut !== value) {
      ...
      if (value !== null) {
        ...
        defines.set(
          "LUT_SIZE",
          Math.min(image.width, image.height).toFixed(16)
        );
...
var OutlineEffect = class extends Effect {
  constructor(
    ...
    this.renderTargetOutline = new WebGLRenderTarget18(1, 1, {
      depthBuffer: false,
    });
...
import {
  ...
  Vector2 as Vector223,
  Vector4 as Vector43,
} from "three";

// src/effects/glsl/pixelation.frag
var pixelation_default =
...
import {
  ...
  Vector2 as Vector225,
  Vector3 as Vector35,
} from "three";

// src/effects/glsl/shock-wave.frag
var shock_wave_default =
...
var SelectiveBloomEffect = class extends BloomEffect {
  constructor(scene, camera, options) {
    ...
    this.renderTargetMasked = new WebGLRenderTarget19(1, 1, {
      depthBuffer: false,
    });
...
var SMAAEffect = class extends Effect {
  constructor({
    ...
    super("SMAAEffect", smaa_default, {
      ...
      blendFunction,
      attributes: EffectAttribute.CONVOLUTION | EffectAttribute.DEPTH,
      uniforms: /* @__PURE__ */ new Map([["weightMap", new Uniform44(null)]]),
    });
    ...
    this.renderTargetEdges = new WebGLRenderTarget20(1, 1, {
      depthBuffer: false,
    });
...
import {
  BasicDepthPacking as BasicDepthPacking18,
  Color as Color9,
...
import {
  Uniform as Uniform46,
  UnsignedByteType as UnsignedByteType17,
} from "three";
...
import {
  sRGBEncoding as sRGBEncoding16,
  Uniform as Uniform47,
  Vector2 as Vector226,
  WebGLRenderTarget as WebGLRenderTarget22,
} from "three";

// src/effects/glsl/tilt-shift.frag
var tilt_shift_default =
  "#ifdef FRAMEBUFFER_PRECISION_HIGH\nuniform mediump sampler2D map;\n#else\nuniform lowp sampler2D map;\n#endif\nuniform vec2 maskParams;varying vec2 vUv2;float linearGradientMask(const in float x){return step(maskParams.x,x)-step(maskParams.y,x);}void mainImage(const in vec4 inputColor,const in vec2 uv,out vec4 outputColor){float mask=linearGradientMask(vUv2.y);vec4 texel=texture2D(map,uv);outputColor=mix(texel,inputColor,mask);}";

// src/effects/glsl/tilt-shift.vert
var tilt_shift_default2 =
...
import {
  ...
  Uniform as Uniform48,
  WebGLRenderTarget as WebGLRenderTarget23,
} from "three";

// src/effects/glsl/tone-mapping.frag
var tone_mapping_default =
...
import {
  ...
  Loader as Loader2,
  LoadingManager as LoadingManager4,
  Vector3 as Vector37,
} from "three";
